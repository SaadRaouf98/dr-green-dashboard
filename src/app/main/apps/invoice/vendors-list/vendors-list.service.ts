...
@Injectable()
export class VendorsListService implements Resolve<any> {
  apiData: any;
  ...
  addVendor(token, obj) {
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + token).set('Accept', 'application/json');
    return this._httpClient.post(`${environment.domain}/admin/suppliers`,
        obj, {headers});
  }
  editVendor(token,id, obj) {
    const headers = new HttpHeaders().set('Authorization', 'Bearer ' + token).set('Accept', 'application/json');
    return this._httpClient.post(`${environment.domain}/admin/suppliers/${id}`,
        obj, {headers});
  }
